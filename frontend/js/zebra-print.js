...
const ZebraPrint = (() => {
  ...
  function buildMap(dados) {
    ...
    const map = {
      ...
      '{VENCIMENTO_DEBITOS}':   formatData(dados.vencimento_notificacao || ''),
      '{VALOR_DEBITOS}':        formatarValor(dados.valor_notificacao || 0),
    ...
    // Código de barras de reaviso/notificação (Alternativa A)
    let rawBarcodeNotif = (dados.codigo_barras_notificacao || '').replace(/\D/g, '').trim();
    if (rawBarcodeNotif.length >= 20) {
        if (rawBarcodeNotif.length % 2 !== 0) rawBarcodeNotif = '0' + rawBarcodeNotif;
        map['{CODIGO_BARRAS_DEBITOS}'] = rawBarcodeNotif;
        if (rawBarcodeNotif.length === 44 && rawBarcodeNotif[0] === '8') {
            map['{LINHA_DIGITAVEL_DEBITOS}'] = formatarLinhaDigitavelConcessionaria(rawBarcodeNotif);
        } else if (rawBarcodeNotif.length === 44) {
            map['{LINHA_DIGITAVEL_DEBITOS}'] = formatarLinhaDigitavel(rawBarcodeNotif);
        } else {
            map['{LINHA_DIGITAVEL_DEBITOS}'] = rawBarcodeNotif;
        }
    } else {
        map['{CODIGO_BARRAS_DEBITOS}'] = '';
        map['{LINHA_DIGITAVEL_DEBITOS}'] = '';
    }
  ...
  function layoutGenerico(dados) {
    return [
      ...
      'LINE 2 178 100 178 0.2',
      'T 7 0 3 179 FAVOR AUTENTICAR NO VERSO',
      'T 5 0 68 179 EMISSAO: {DATA_EMISSAO}',
      'T 5 0 68 182 HORA: {HORA_EMISSAO}',
      'LINE 2 184 100 184 0.2',
      'LINE 2 184 2 212 0.2',
      'LINE 100 184 100 212 0.2',
      'LINE 2 200 100 200 0.2',
      ...
      'T 7 0 3 197 Seq. ant: {SEQUENCIA}',
      'T 7 2 78 185 MES/ANO: {REFERENCIA}',
      'LINE 76 188 100 188 0.2',
      'T 7 0 78 189 NR. GUIA ',
      'T 7 0 78 192 {NR_GUIA}',
      'LINE 76 195 100 195 0.2',
      'T 7 0 78 196 CATEGORIA ',
      'T 7 0 82 199 {CATEGORIA}',
      'T 7 0 12 201 LIGACAO ',
      'T 7 0 12 204 {LIGACAO}',
      'LINE 35 200 35 212 0.2',
      'T 7 0 47 201 VENCIMENTO ',
      'T 7 0 47 204 {DATA_VENCIMENTO}',
      'T 7 0 80 201 VALOR PAGO ',
      'T 7 0 82 204 R$ {VALOR_PAGAR}',
      'LINE 76 184 76 212 0.2',
      'LINE 2 212 100 212 0.2',
      'LEFT',
      'T 0 0 5 213 {LINHA_DIGITAVEL}',
      'B I2OF5 0.245 25 8 5 220 {CODIGO_BARRAS}',
      'FORM',
      'PRINT'
    ].join('\r\n');
  }
