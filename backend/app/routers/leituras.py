...
async def list_clientes_com_leituras(
    ...
    for c in clientes:
        ...
        items.append(ClienteComLeitura(
            ...
            tem_notificacao=c.tem_notificacao,
            vencimento_notificacao=c.vencimento_notificacao,
            valor_notificacao=float(c.valor_notificacao) if c.valor_notificacao else 0.0,
            codigo_barras_notificacao=c.codigo_barras_notificacao,
            lote=getattr(c, 'lote', None),
            quadra=getattr(c, 'quadra', None),
            codigo_baixa=getattr(c, 'codigo_baixa', None),
            debitos_notificacao=getattr(c, 'debitos_notificacao', None),
            tem_agua=c.tem_agua,
