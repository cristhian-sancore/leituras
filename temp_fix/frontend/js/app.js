...
async function loadPrecos() {
    ...
    try {
        ...
        const grouped = {};
        tarifas.forEach(t => {
            if (!grouped[t.categoria]) grouped[t.categoria] = { agua: [], lixo: [], esgoto: [] };
        ...
        for (const [cat, servicos] of Object.entries(grouped)) {
            ...
            if (servicos.esgoto.length) {
                const min = servicos.esgoto.find(f => f.valor_minimo);
                html += `<p style="font-size:0.85rem"><strong>Esgoto Mín:</strong> ${min ? fmtMoeda(min.valor_minimo) : '-'}</p>`;
            }
...
// Nota: desativarUsuario foi removida (duplicata) — usar toggleUsuario(id, true)
